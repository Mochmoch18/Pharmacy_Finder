...
cities = ["marrakech","casablanca","kenitra","rabat","sale"]  # Start with one city for testing "casablanca","kenitra","rabat","sale"
                ...
                if district_response.status_code == 200:
                    ...
                    if pharmacy_list:
                        ...
                        for item in pharmacy_items:
                            name = item.find("h3", itemprop="name").get_text(strip=True) if item.find("h3",
                                                                                                      itemprop="name") else "N/A"
                            initial_address = item.find("p", itemprop="streetAddress").get_text(
                                strip=True) if item.find("p", itemprop="streetAddress") else "N/A"
                            garde_status = item.find("span", class_="garde_status").get_text(strip=True) if item.find(
                                "span", class_="garde_status") else "N/A"
                            pharmacy_link = item.find("a", href=True)["href"] if item.find("a", href=True) else None

                            # Visit individual pharmacy page to get the full phone number and detailed address
                            full_address = initial_address
                            phone = "N/A"
                            if pharmacy_link:
                                ...
                                if pharmacy_response.status_code == 200:
                                    ...
                                    phone = pharmacy_soup.find("a", itemprop="telephone").get_text(
                                        strip=True) if pharmacy_soup.find("a", itemprop="telephone") else "N/A"

                                    # Get the full address from the pharmacy page, if available
                                    detailed_address = pharmacy_soup.find("p", itemprop="streetAddress")
                                    if detailed_address:
                                        full_address = detailed_address.get_text(strip=True)

                                print(f"Retrieved phone: {phone}")
                                print(f"Retrieved address: {full_address}")

                            # Check if any field contains 'N/A' and skip those rows
                            if name != "N/A" and full_address != "N/A" and phone != "N/A" and garde_status != "N/A":
                                # Write data to CSV, including district name
                                writer.writerow(
                                    [city.capitalize(), district_name, name, full_address, phone, garde_status])
